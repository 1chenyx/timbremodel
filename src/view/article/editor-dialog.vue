<template>
  <el-dialog title="上传模型" :modal-append-to-body="false" :before-close="handleClose" v-model="dialogFormVisible"
    :close-on-click-modal="false" width="40%" top ="7vh" :fullscreen="fullscreen">
    <div>
      <el-form label-width="100px" :rules="rules" :model="form" status-icon ref="form" @submit.native.prevent>
        <el-row>
          <el-col :lg="24">
            <el-form-item label="标题" prop="title">
              ...
              <el-input class="editor-title" size="default" v-model="form.title" placeholder="请填写模型标题"></el-input>
            </el-form-item>
          </el-col>
          <el-col :lg="24">
            <el-form-item label="模型类型" prop="channel_id">
              <el-select class="editor-select" size="default" filterable v-model="form.channel_id" :disabled="channels.length === 0"
                placeholder="请选择模型类型">
                <el-option v-for="item in channels" :key="item.id" :label="item.channel_name" :value="item.id">
                </el-option>
              </el-select>
        ...
        <el-row>
          ...
          <el-col :lg="12">
            <el-form-item label="模型来源" prop="excerpt">
              <el-select v-model="form.article_type" filterable default-first-option placeholder="请选择模型来源">
        ...
        <el-row>
          ...
          <el-col :lg="12">
            <el-form-item label="效果语音">
              <upload-files  :multiple="false"  :max-num="1" @successHandel="successHandel" @errorHandel="errorHandel"/>
            </el-form-item>
          </el-col>
          <el-col :lg="24">
            <el-form-item label="音频地址" prop="audio_path">
              <el-input size="default" disabled  :autosize="{ minRows: 4, maxRows: 8 }" placeholder="上传音频自动填充"
                v-model="form.audio_path"></el-input>
            </el-form-item>
          </el-col>
          <el-col :lg="16">
            <el-form-item label="下载地址" prop="download_path">
              <el-input size="default"  :autosize="{ minRows: 4, maxRows: 8 }" placeholder="请填写下载地址"
                v-model="form.download_path"></el-input>
            </el-form-item>
          </el-col>
          <el-col :lg="8">
            <el-form-item label="下载密码" prop="download_pass">
              <el-input size="default"  :autosize="{ minRows: 4, maxRows: 8 }" placeholder="请填写下载密码"
                v-model="form.download_pass"></el-input>
            </el-form-item>
          </el-col>
          <el-col :lg="24">
...
export default {
  name: 'EditorDialog',
  components: { UploadImgs , UploadFiles },
  props: {
    id: [String, Number],
    title: [String],
    content: [String],
  ...
  methods: {
    ...
    successHandel(reslut) {
      this.form.audio_path = reslut && reslut.url
    },
    errorHandel() {
      this.form.audio_path = ""
    },
    toIndex() {
      this.handleClose()
    }
...
.el-col-lg-12 .el-form-item__content .el-select {
  width: 100%;
}
.editor-title, .editor-select{
